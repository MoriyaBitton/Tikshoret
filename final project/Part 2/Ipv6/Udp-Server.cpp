...
#define SERVER_IP_ADDRESS "::1" // local host
...
int main()
{
	...
	// Create socket
	if ((s = socket(PF_INET6 /*Ipv6*/, SOCK_DGRAM, IPPROTO_UDP)) == -1) // In Windows -1 is SOCKET_ERROR
	{
	...
	struct sockaddr_in6 /*Ipv6*/ serverAddress;
	memset((char *)&serverAddress, 0, sizeof(serverAddress));
	serverAddress.sin6_family /*Ipv6*/ = PF_INET6 /*Ipv6*/;
	serverAddress.sin6_port /*Ipv6*/ = htons(SERVER_PORT);
	inet_pton(PF_INET6 /*Ipv6*/, (const char *)SERVER_IP_ADDRESS, &(serverAddress.sin6_addr /*Ipv6*/));
	...
	struct sockaddr_in6 /*Ipv6*/ clientAddress;
	...
	while (1)
	{
		...
		inet_ntop(PF_INET6 /*Ipv6*/, &clientAddress.sin6_addr /*Ipv6*/, clientIPAddrReadable, sizeof(clientIPAddrReadable));

		// print details of the client/peer and the data received
		printf("Received packet from %s:%d\n", clientIPAddrReadable, ntohs(clientAddress.sin6_port /*Ipv6*/));
		printf("Data is: %s\n", buffer);
